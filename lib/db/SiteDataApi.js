...
import { reorderPublishedPostsInAllPagesByTopTag } from '@/lib/utils/pinnedPosts'
...
async function convertNotionToSiteData(
  ...
  // 全局置顶：开启 TOP_TAG 时，在 allPages 上对「已发布 Post」子序列重排（带标签在前），
  // 列表页 / 分页 / 分类 / 标签 / 搜索等只需按原顺序 filter 即可。
  const topTag = siteConfig('TOP_TAG', '', NOTION_CONFIG)
  if (topTag) {
    allPages = reorderPublishedPostsInAllPagesByTopTag(allPages, topTag)
  }
