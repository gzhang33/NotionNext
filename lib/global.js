...
import { isArticleDetailNavigationPath } from './utils/articleRouteDetection'
import ArticleRouteSkeleton from '@/components/ArticleRouteSkeleton'
...
export function GlobalContextProvider(props) {
  ...
  /** 文章详情类路由切换时展示全局骨架，避免主内容区短暂空白 */
  const [showArticleRouteSkeleton, setShowArticleRouteSkeleton] =
    useState(false)
  ...
  useEffect(() => {
    const handleStart = (url, options = {}) => {
      ...
      if (options.shallow) {
        setShowArticleRouteSkeleton(false)
        return
      }
      try {
        const path = new URL(url, window.location.href).pathname
        setShowArticleRouteSkeleton(isArticleDetailNavigationPath(path))
      } catch {
        setShowArticleRouteSkeleton(false)
      }
    }
  ...
  return (
    <GlobalContext.Provider
      value={contextValue}>
      {children}
      <ArticleRouteSkeleton visible={showArticleRouteSkeleton} />
    </GlobalContext.Provider>
  )
}
