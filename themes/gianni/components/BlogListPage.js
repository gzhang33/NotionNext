...
export default function BlogListPage(props) {
  ...
  const GIANNI_POST_AD_ENABLE = siteConfig('GIANNI_POST_AD_ENABLE', false, CONFIG)
  ...
  return (
    <div className='w-full mb-12'>
      <div id='posts-wrapper' ref={containerRef}>
        {posts?.map((p, index) => (
          <div key={p.id}>
            {GIANNI_POST_AD_ENABLE && (index + 1) % 3 === 0 && <AdSlot type='in-article' />}
            {GIANNI_POST_AD_ENABLE && index + 1 === 4 && <AdSlot type='flow' />}
      ...
      <div className='flex justify-between items-center mt-4 pt-4' style={{ borderTop: '1px solid var(--divider)' }}>
        <SmartLink
          ...
          <i className='fa-solid fa-arrow-left mr-1' /> {locale.PAGINATION.PREV}
        </SmartLink>
        ...
        <SmartLink
          ...
          {locale.PAGINATION.NEXT} <i className='fa-solid fa-arrow-right ml-1' />
        </SmartLink>
      </div>
    </div>
  )
